...
// 辅助函数：路径转换 (与 FenceWindow 逻辑保持一致)
static QString toStoragePath(const QString& path, const QString& fenceId) {
    ...
    QDir base(storageBase);
    if (normalizedPath.startsWith(storageBase, Qt::CaseInsensitive)) {
        QString relative = base.relativeFilePath(normalizedPath);
...
void MainWindow::loadFences()
{
    ...
    for (const QJsonValue &val : fencesArray) {
        ...
        QString id = fenceObj["id"].toString();
        if (!id.isEmpty()) fence->setId(id);
        ...
        // 加载图标
        QJsonArray iconsArray = fenceObj["icons"].toArray();
        for (const QJsonValue &iconVal : iconsArray) {
            QJsonObject iconObj = iconVal.toObject();
            IconWidget::IconData data;
            data.name = iconObj["name"].toString();
            QString savedPath = iconObj["path"].toString();
            data.path = fromStoragePath(savedPath, id);
            ...
            if (fi.exists()) {
                data.icon = iconProvider.icon(fi).pixmap(48, 48);
...
void MainWindow::saveFences()
{
    ...
    for (FenceWidget *fence : m_fences) {
        ...
        for (IconWidget *icon : fence->icons()) {
            ...
            iconObj["name"] = data.name;
            iconObj["path"] = toStoragePath(data.path, fence->id());
