...
void FenceWidget::removeIcon(IconWidget *icon)
{
    if (icon && m_icons.contains(icon)) {
        ...
        if (icon->data().isFromDesktop) {
            ...
            qDebug() << "    [FenceWidget] Restoring icon to desktop. srcPath:" << srcPath << "targetPath:" << targetPath;

            // 移动文件回去
            if (QFile::exists(srcPath)) {
                bool restored = false;
                if (QFile::exists(targetPath)) {
                    qDebug() << "    Target already exists on desktop, skipping move/copy.";
                    if (srcPath != targetPath) {
                        QFile::remove(srcPath);
                    }
                    restored = true;
                } else {
                    bool ok = QFile::rename(srcPath, targetPath);
                    if (!ok) {
                        qDebug() << "    Restore rename failed, trying Qt copy...";
                        if (QFile::copy(srcPath, targetPath)) {
                            ok = true;
                        } else {
                            qDebug() << "    Restore Qt copy failed, trying WinAPI CopyFileW...";
                            if (CopyFileW((const wchar_t*)srcPath.utf16(), (const wchar_t*)targetPath.utf16(), FALSE)) {
                                ok = true;
                            }
                        }
                        ...
                        if (ok) {
                            if (!QFile::remove(srcPath)) {
                                qDebug() << "    Restore copy succeeded but source cleanup failed.";
                            }
                        }
                    }
                    restored = ok;
                ...
                if (restored) {
                    qDebug() << "    Restore SUCCESS! Notifying shell...";
                    DesktopHelper::notifyFileAdded(targetPath);
                    
                    // 强制刷新桌面目录
                    SHChangeNotify(SHCNE_UPDATEDIR, SHCNF_PATHW, QDir::toNativeSeparators(desktopPath).toStdWString().c_str(), NULL);
                    
                    // 尝试设置原始位置
                    QPoint originalPos = icon->data().originalPosition;
                    if (originalPos.x() >= 0 && originalPos.y() >= 0) {
                        QTimer::singleShot(1000, [targetPath, originalPos]() {
...
void FenceWidget::dragMoveEvent(QDragMoveEvent *event)
{
    if (event->mimeData()->hasFormat("application/x-deskgo-icon") || event->mimeData()->hasUrls()) {
        event->setDropAction(Qt::MoveAction);
        event->accept();
...
void FenceWidget::dropEvent(QDropEvent *event)
{
    ...
    if (mimeData->hasFormat("application/x-deskgo-icon")) {
        ...
        for (const QUrl &url : mimeData->urls()) {
            if (url.isLocalFile()) {
                ...
                data.path = path;
                data.name = fi.completeBaseName();
                if (path.endsWith(".lnk", Qt::CaseInsensitive)) {
                    data.name = fi.fileName();
                    if (data.name.endsWith(".lnk", Qt::CaseInsensitive)) data.name.chop(4);
                }
                
                // 检测是否来自桌面
                QString userDesktop = QDir::toNativeSeparators(QStandardPaths::writableLocation(QStandardPaths::DesktopLocation));
                QString publicDesktop = QDir::toNativeSeparators("C:/Users/Public/Desktop");
                
                if (path.startsWith(userDesktop, Qt::CaseInsensitive) || 
                    path.startsWith(publicDesktop, Qt::CaseInsensitive)) {
                    data.isFromDesktop = true;
                    data.originalPosition = DesktopHelper::getIconPosition(path);
                }
    ...
    if (window()) {
        QTimer::singleShot(100, [this]() {
            // 这里通常是通过 MainWindow 的某种机制保存，但 FenceWidget 自身没有 save 信号
            // 不过 MainWindow 在析构时会保存。为了保险，我们可以发出一个自定义信号。
        });
    }
}
